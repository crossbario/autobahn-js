...
Factory.prototype.create = function () {
   ...
   if (global.process && global.process.versions.node
      ...
      (function () {
         ...
         if (self._options.autoping && self._options.autoping >= 0) {
            options.ping_interval = self._options.autoping
         }
         ...
         websocket.on('open', function () {
            ...
            if (options.ping_interval) {
                websocket.pingIntervalId = setInterval(function () {
         ...
         websocket.on('close', function (code, message) {
            if (websocket.pingIntervalId) clearInterval(websocket.pingIntervalId);
            var details = {
               code: code,
               reason: message,
               wasClean: code === 1000
